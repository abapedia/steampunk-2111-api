INTERFACE if_xco_i18n_me_prm_target PUBLIC.

  DATA:
    metadata_extension TYPE REF TO if_xco_metadata_extension READ-ONLY,
    parameter_name     TYPE sxco_cds_parameter_name READ-ONLY.

  METHODS:
    get_translations
      IMPORTING
        it_text_attributes     TYPE sxco_t_me_prm_text_attributes
      RETURNING
    ...
    set_translation
      IMPORTING
        io_language        TYPE REF TO if_xco_language
        it_texts           TYPE sxco_t_me_prm_texts
        io_change_scenario TYPE REF TO if_xco_cts_change_scenario,

    has_translation
      IMPORTING
        io_language               TYPE REF TO if_xco_language
        io_text_attribute         TYPE REF TO cl_xco_me_prm_text_attribute
      RETURNING
